            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                    ...
                    <div className={`lg:col-span-4 rounded-[3rem] p-8 sm:p-10 h-fit flex flex-col ${isHighContrast ? 'border-4 border-white bg-black' : 'bg-zinc-900 shadow-premium-lg'}`}>
                        <h3 className="text-h4 mb-8 flex items-center border-b border-white/10 pb-6 text-white">
                        ...
                        <ul className="space-y-8 flex-grow">
                            {currentLang.notices.map((notice: any, idx: number) => (
                                <li key={idx} className={`pb-8 ${idx !== currentLang.notices.length - 1 ? 'border-b border-white/10' : ''}`}>
                                    <span className={`text-ui-label block mb-3 font-mono font-bold uppercase ${isHighContrast ? 'text-white' : 'text-zinc-500'}`}>
                                        {notice.date}
                                    </span>
                                    <a href="#" className={`text-h4 font-bold leading-tight transition-colors line-clamp-3 block ${isHighContrast ? 'text-white underline decoration-2' : 'hover:text-blue-400 text-white/90'}`}>
                                        {notice.title}
                                    </a>
                                </li>
                            ))}
                        </ul>
                        <button className={`w-full mt-8 py-4 rounded-2xl text-ui-label font-bold uppercase transition-all ${isHighContrast ? 'bg-white text-black' : 'bg-white/10 hover:bg-white/20 text-white border border-white/10'}`}>
