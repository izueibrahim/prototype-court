      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`p-8 sm:p-12 rounded-[2.5rem] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border flex flex-col md:flex-row gap-12 ${isHighContrast ? 'bg-black border-white' : 'bg-white border-slate-200'}`}>
          <div className="flex-1 space-y-8">
            <div>
              <h3 className={`text-sm font-extrabold uppercase tracking-widest mb-2 ${isHighContrast ? 'text-slate-400' : 'text-slate-400'}`}>{currentLang.hqTitle}</h3>
              <p className={`text-lg sm:text-xl font-bold leading-tight whitespace-pre-line ${isHighContrast ? 'text-white' : 'text-slate-900'}`}>
                {currentLang.hqAddr}
              </p>
            </div>
            <div>
              <h3 className={`text-sm font-extrabold uppercase tracking-widest mb-2 ${isHighContrast ? 'text-slate-400' : 'text-slate-400'}`}>{currentLang.enquiryTitle}</h3>
          ...
          <div className="flex-1">
            <div className={`w-full h-full min-h-[250px] rounded-2xl flex flex-col items-center justify-center border-2 border-dashed ${isHighContrast ? 'border-slate-800 bg-slate-900/50 text-slate-500' : 'border-slate-200 bg-slate-50 text-slate-400'}`}>
              ...
              <span className="font-bold text-sm">{currentLang.mapTitle}</span>
            </div>
          </div>
        </div>
      </div>
